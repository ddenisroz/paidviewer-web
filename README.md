...
Если backend будет только по IP, используй [frontend/.env.ip-only.example](frontend/.env.ip-only.example) и скопируй [frontend/vercel.ip-only.example.json](frontend/vercel.ip-only.example.json) в `frontend/vercel.json`. Реальный IP не коммить в репозиторий: он хранится только в Vercel Environment Variables `BOT_SERVICE_HTTP_TARGET` и `BOT_SERVICE_WS_TARGET`. Для WebSocket bridge в Vercel должен быть включен Fluid Compute.
