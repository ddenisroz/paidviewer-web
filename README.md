...
Если backend будет только по IP, используй [frontend/.env.ip-only.example](frontend/.env.ip-only.example) и скопируй [frontend/vercel.ip-only.example.json](frontend/vercel.ip-only.example.json) в `frontend/vercel.json`, заменив `YOUR_SERVER_IP`. Для WebSocket bridge в Vercel должен быть включен Fluid Compute.
